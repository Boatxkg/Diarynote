...
@notes.route('/notes', defaults={'search': None}, methods=['GET'])
@notes.route('/notes/<string:search>', methods=['GET'])
@jwt_required()
def get_note(search):
    user_id = get_jwt_identity()
    user = User.objects(id=user_id).first()
    ...
    if search:
        search = search.lower()
        notes = Note.objects(user=user, title__icontains=search).order_by('-created_at')
    else:
        notes = Note.objects(user=user).order_by('-created_at')
